...
describe('createTaskListView', () => {
  ...
  describe('締め切り表示', () => {
    it('deadline設定時、その値が表示される', () => {
      const wrapper = createTaskListView([makeTodo('1', { deadline: '2026-04-30' })], false)
  ...
  describe('チェックボタン', () => {
    it('チェックボタンにdata-id属性が設定される', () => {
      const wrapper = createTaskListView([makeTodo('abc')], false)
      const btn = wrapper.querySelector<HTMLButtonElement>('button[data-id]')
      expect(btn?.dataset['id']).toBe('abc')
    })
